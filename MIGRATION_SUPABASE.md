...
**Statut** : migration **terminée** (25/07/2026). Phases 0→5 complètes : schéma (9 migrations), données (24 627 parfums + images), couche services 100 % Supabase (deps + code Firebase retirés), 6 Edge Functions + 3 crons déployés, E2E 24/24, tsc 0, 216/216 tests, app fonctionnelle sur émulateur (`EXPO_PUBLIC_USE_SUPABASE=true`). Config dashboard effectuée (Google provider avec 2 client IDs + confirmation email off). Reste optionnel : lien expo.dev (`npx eas init`) pour les push device.
